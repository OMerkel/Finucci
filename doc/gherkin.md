...
Given a card with suit "denari", rank "asso", and value 1
When the card is created
Then the card should have suit "denari", rank "asso", and value 1
...
Given two cards with identical suit and rank ("denari", "asso")
When comparing them for equality
Then the comparison should return true

Given two cards with different suits ("denari", "asso") vs ("coppe", "asso")
When comparing them for equality
...
Given a card with suit "spade" and rank "7"
When retrieving the display name
Then the display name should be "7 of spade"
...
Then each of the 4 suits (denari, coppe, spade, bastoni) should have exactly
10 cards
...
Then the deck should contain Asso(1), 2-7, Fante(8), Cavallo(9), Re(10) in
each suit
...
Then Asso should equal 1, 2-7 should equal face value, Fante should equal 8,
Cavallo should equal 9, Re should equal 10
...
Then the phase should be "setup", round should be 1, and both players should
have score 0
...
### Scenario: Primiera Method Configuration
...
### Scenario: Find Valid Exact-Match Captures

Feature: Capture Validation (FR-4.1)

Given a played card value 7 and table cards [7, 3, 4]
When getting valid captures
Then valid captures should include [7] (exact match)

### Scenario: Single-Card Capture

Feature: Single Exact-Match Capture (FR-4.1)

Given a played card 5 and table [5, 2, 3]
When getting valid captures
Then valid captures should include [5] (single-card exact match)

### Scenario: Multi-Card Capture

Feature: Multi-Card Combination (FR-4.1)

Given a played card 5 and table [2, 3, 8]
When getting valid captures
Then valid captures should include [2, 3] (sum equals played value)
...
Given played card 6 and table [2, 4]
When checking hasCapture
Then hasCapture should return true

### Scenario: Exact-Match Priority Enforcement

Feature: Exact-Match Priority (FR-4.4)

Given captures include both [5] (single exact match) and [2, 3] (multi-card sum)
When applying exact-match priority
Then only [5] should remain as valid capture

---

## test-scopa.test.js

### Scenario: Detect Scopa When Table Cleared

Feature: Scopa Detection (FR-6.1)

Given table with 1 card (value 7)
When capturing with played card (value 7)
Then isScopa should return true

### Scenario: No Scopa When Table Partially Captured

Feature: Partial Capture (FR-6.1)

Given table with 3 cards and capturing only 2
When checking isScopa
Then isScopa should return false

### Scenario: Award Scopa Point to Player

Feature: Scopa Point Attribution (FR-6.2)

Given a player with 0 scope
When awarding a scopa
Then scope count should increment to 1

### Scenario: Automatic Final Table Award Is Not Scopa
...
And no scopa should be awarded

### Scenario: Intermediate-Hand Sweep Still Scores Scopa

Feature: Scopa Detection Across Re-deals (FR-6.4)

Given house rule "no scopa on final card" enabled
...
Then scopa should still be awarded

### Scenario: Final-Round Sweep Does Not Score Under House Rule

Feature: Final Card of Round Scopa Suppression (FR-6.4, FR-11.3)

Given house rule "no scopa on final card" enabled
And a table-clearing capture using a player's last card in hand
And stock is exhausted
When resolving the capture
Then scopa should NOT be awarded
...
### Scenario: Score Denari Category (Majority)

Feature: Denari Scoring (FR-8.2)

Given player 1 with 6 denari, player 2 with 4 denari
When scoring denari category
Then player 1 should receive 1 point

### Scenario: Score Settebello (7 of Denari)

Feature: Settebello Scoring (FR-8.3)

Given player 1 has the 7 of denari card
When scoring settebello category
Then player 1 should receive 1 point

### Scenario: Score Scope

Feature: Scopa Points (FR-8.5)

Given player 1 with 3 scope made, player 2 with 1
When scoring scope
...
Given final captures and scope for both players
When calculating round score
Then cumulative points should equal: cards + denari + settebello + primiera + scope
...
Given played card 5, selected capture [2, 3]
When validating move
Then validation should pass when selected capture is [2, 3] and sum is 5
...
When validating move
Then validation should fail (6 does not equal 5)
...
And no scopa point awarded for final award
...
Then AI should select scopa capture
...
When AI calculates move (greedy strategy)
Then AI should select 7 of denari capture
...
Given a game state
When running MCTS
Then algorithm should perform Selection → Expansion → Simulation →
...
### Scenario: Calculate Average Scope Per Round

Feature: Average Scopa Calculation (FR-14.4)

Given 5 rounds with scope counts [2, 1, 3, 0, 2]
When calculating average
Then average should be 1.6 scope per round
...
Then board should display table cards, player hands, scores, and current
player indicator
...
Then display should show player names, scores, win rates, scope counts
...
Given configuration with primieraMethod "numerical"
When scoring round
Then 7=21, 6=18, Asso=16 values should apply (not prime method)

### Scenario: Enable/Disable Final Card Scopa Rule
