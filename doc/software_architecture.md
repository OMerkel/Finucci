# Scopa - Software Architecture

## Architecture Overview

This document defines the software architecture for a digital implementation of
Scopa in HTML5/JavaScript. The architecture is organized around
...
- resolving explicit rule profiles (`classic_scopa` vs `digital_default`) into
  effective runtime options;
- supporting deterministic seeded shuffle mode for test/replay workflows;
...
- **Rule Profiles**: runtime options are derived from explicit profiles so
  defaults remain unambiguous across releases.
- **Deterministic Replay Mode**: seeded shuffle mode supports reproducible
  testing and SGF replay while leaving production shuffle behavior fair and
  non-deterministic by default.
- **Primiera Determinism**: default primiera method and tie-break behavior are
  explicit in configuration and scoring paths.
...
- **Shared Scopa Rule Helper**: table-clearing captures are filtered through a
  central scopa-scoring helper so the engine, previews, and AI all agree on
...
- **Requirement Coverage Gate**: CI validates requirement-to-test mappings for
  active FR/NFR items.
- **Cross-Document Rule Parity**: architecture, requirements, gherkin, and
  rulebook documents are treated as synchronized normative sources.
...
implementation of Scopa in HTML5/JavaScript. It organizes code into
