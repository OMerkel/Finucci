...
- capture legality, scopa behavior, house-rule edge cases, and scoring;
...
- final-card-of-round scopa suppression semantics, including the distinction
...
functional requirement paths, with full regression coverage on critical gameplay
flows.

Required baseline for functional requirement coverage aligns with
`requirements.md`: at least 90 percent line/branch coverage for FR paths, with
critical gameplay paths kept at full regression coverage.
...
- **Requirement Coverage Gate (NFR-2.4)**: CI validates that active FR/NFR
  entries are mapped to automated tests; unmapped requirement changes fail CI.
...
- **Code Structure**: layered architecture separates UI, logic, AI, and
  persistence.
- **Cross-Document Rule Parity (NFR-3.3)**: rule semantics are synchronized
  across `rules.md`, `regeln.md`, `requirements.md`, and `gherkin.md` through
  parity checks and coordinated documentation updates.
...
in browser `localStorage` (`scopa_stats`), which keeps deployment backend-free
...
- **Documentation drift risk**: mitigated by splitting the architecture into
  module, runtime, quality, and diagram views, plus explicit parity checks.
