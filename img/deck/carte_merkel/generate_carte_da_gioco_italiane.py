"""Generate a traditional-style 40-carte da gioco italiane SVG deck."""
...
class Suit:
    """Color and naming metadata for one Italian suit."""
...
SUITS = (
    Suit(
        "denari", "Denari", "denari", "#c28a1e", "#784b0b",
        "#f4d78a", "#8f6713", "#9f7825"
    ),
    Suit(
        "coppe", "Coppe", "coppe", "#a3352f", "#5d1717",
        "#e9b4a8", "#7f2424", "#8f3a30"
    ),
    Suit(
        "spade", "Spade", "spade", "#436b83", "#243746",
        "#bdd0dc", "#39596f", "#5d8497"
    ),
    Suit(
        "bastoni", "Bastoni", "bastoni", "#5c6b2d", "#314116",
        "#b4c08b", "#556227", "#6d5526"
    ),
)


RANKS = (
    Rank("asso", "1", "Asso", 1, False),
    Rank("2", "2", "Due", 2, False),
    Rank("3", "3", "Tre", 3, False),
    Rank("4", "4", "Quattro", 4, False),
    Rank("5", "5", "Cinque", 5, False),
    Rank("6", "6", "Sei", 6, False),
    Rank("7", "7", "Sette", 7, False),
    Rank("fante", "10", "Fante", 10, True),
    Rank("cavallo", "11", "Cavallo", 11, True),
    Rank("re", "12", "Re", 12, True),
)
...
def svg_document(body: str, title: str) -> str:
  ...
  <title id="title">{title}</title>
    <desc id="desc">Traditional-style carte da gioco italiane.</desc>
  <defs>
...
def coin_symbol(suit: Suit, x: float, y: float, scale: float = 1.0, rotate: float = 0) -> str:
    """Render a Denari pip at the requested position."""
...
def cup_symbol(suit: Suit, x: float, y: float, scale: float = 1.0, rotate: float = 0) -> str:
    """Render a Coppe pip at the requested position."""
...
def sword_symbol(suit: Suit, x: float, y: float, scale: float = 1.0, rotate: float = 0) -> str:
    """Render a Spade pip at the requested position."""
...
def baton_symbol(suit: Suit, x: float, y: float, scale: float = 1.0, rotate: float = 0) -> str:
    """Render a Bastoni pip at the requested position."""
...
def number_body(suit: Suit, rank: Rank) -> str:
    ...
    # Special rotation overrides for center pip on rank 3 for bastoni and spade
    center_rotate = -45 if (rank.key == "3" and suit.key in ("bastoni", "spade")) else 0
    symbols = "\n".join(
...
def face_body(suit: Suit, rank: Rank) -> str:
    ...
    if rank.key == "re":
        # Suit symbol at ~1/2 of the 480px emoji size, lower-left, rotated -30°.
        re_suit = suit_symbol(suit, 190, 840, 2.6, -30)
    ...
    <text x="{format_number(CENTER_X)}" y="300" text-anchor="middle" dominant-baseline="middle" font-size="88" font-family="Georgia, Times New Roman, serif" font-weight="700" fill="#f8eed6" letter-spacing="8">RE</text>'''

    if rank.key == "cavallo":
        cavallo_suit = suit_symbol(suit, 190, 840, 2.6, -30)
        return f'''  <ellipse cx="{format_number(CENTER_X)}" cy="{format_number(CENTER_Y)}" rx="160" ry="240" fill="#fbf6ec" stroke="{suit.banner}" stroke-width="4"/>
    <text x="{format_number(CENTER_X)}" y="660" text-anchor="middle" dominant-baseline="middle" font-size="480">&#x1F3C7;</text>
    {cavallo_suit}
    <rect x="165" y="230" width="420" height="105" rx="12" fill="{suit.dark}" opacity="0.72"/>
    <text x="{format_number(CENTER_X)}" y="300" text-anchor="middle" dominant-baseline="middle" font-size="72" font-family="Georgia, Times New Roman, serif" font-weight="700" fill="#f8eed6" letter-spacing="6">CAVALLO</text>'''

    if rank.key == "fante":
        fante_suit = suit_symbol(suit, 190, 840, 2.6, -30)
        return f'''  <ellipse cx="{format_number(CENTER_X)}" cy="{format_number(CENTER_Y)}" rx="160" ry="240" fill="#fbf6ec" stroke="{suit.banner}" stroke-width="4"/>
    <text x="{format_number(CENTER_X)}" y="660" text-anchor="middle" dominant-baseline="middle" font-size="480">&#x1F472;</text>
    {fante_suit}
    <rect x="200" y="230" width="350" height="110" rx="12" fill="{suit.dark}" opacity="0.72"/>
    <text x="{format_number(CENTER_X)}" y="300" text-anchor="middle" dominant-baseline="middle" font-size="88" font-family="Georgia, Times New Roman, serif" font-weight="700" fill="#f8eed6" letter-spacing="8">FANTE</text>'''
...
def card_svg(suit: Suit, rank: Rank) -> str:
    ...
    title = f"{rank.name} di {suit.name}"
    body = "\n".join(
        (
            card_frame(),
            ace_body(suit) if rank.key == "asso" else (face_body(suit, rank) if rank.face else number_body(suit, rank)),
        )
    )
...
def back_svg(title: str) -> str:
    ...
    <text x="375" y="280" text-anchor="middle" font-size="104" font-family="Georgia, Times New Roman, serif" letter-spacing="8" fill="#f8eed6" font-weight="700">CARTE</text>
  ...
  <path d="M375 384 L425 524 L550 524 L450 594 L500 734 L375 664 L250 734 L300 594 L200 524 L325 524 Z" fill="#ecd5a4" stroke="#f8eed6" stroke-width="5"/>
    <text x="375" y="950" text-anchor="middle" font-size="92" font-family="Georgia, Times New Roman, serif" letter-spacing="6" fill="#f8eed6">ITALIANE</text>'''
    return svg_document(body, title)
...
def contact_sheet_item(index: int, suit: Suit, rank: Rank) -> str:
    ...
    label = f"{rank.name} di {suit.name}"
...
def contact_sheet(suits: tuple[Suit, ...], ranks: tuple[Rank, ...]) -> str:
    ...
    <image href="carte_da_gioco_italiane_back.svg" x="0" y="0" width="{SHEET_THUMB_WIDTH}" height="{SHEET_THUMB_HEIGHT}"/>
    ...
    <text x="{round(SHEET_WIDTH / 2)}" y="52" text-anchor="middle" font-size="32" font-family="Georgia, Times New Roman, serif" letter-spacing="4" fill="#4e3723">CARTE DA GIOCO ITALIANE</text>
...
def generate(output_dir: Path) -> None:
    ...
    write_text(output_dir / "carte_da_gioco_italiane_back.svg", back_svg("Carte da Gioco Italiane Card Back"))
    write_text(output_dir / "carte_da_gioco_italiane_sheet.svg", contact_sheet(SUITS, RANKS))
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate a traditional-style 40-card carte da gioco italiane SVG deck."
    )
    parser.add_argument(
        "output_dir",
        nargs="?",
