/**
 * Persistence Module
 * Handles: SGF game export/import, statistics tracking
 * Requirements: FR-14 (Statistics), FR-16 (SGF Format)
 */
...
export function parseSGF(sgfString) {
  ...
  try {
    // Simple SGF parser for Scopa (subset of SGF format)
    // Format: (;GM[Scopa]P1[name]P2[name]H1[Asso of denari,2 of coppe]...
 ...
 * Format: "Asso of denari, 2 of coppe, 3 of spade"
 *
...
export class PlayerStats {
   ...
   *
   * @param {Object} roundResult - {won: boolean, score: number, Scope: number}
   */
  recordRound(roundResult) {
    this.roundsPlayed += 1;
    if (roundResult.won) this.roundsWon += 1;
    this.totalScore += roundResult.score || 0;
    this.totalScope += roundResult.Scope || 0;
    this.cardsWon += roundResult.cards || 0;
    this.orosWon += roundResult.denari || 0;
  }
  ...
  /**
   * Get average Scope per round
   * FR-14.4: Calculate average Scope
   *
   * @returns {number} Average Scope
   */
  getAverageScope() {
    if (this.roundsPlayed === 0) return 0;
    return +(this.totalScope / this.roundsPlayed).toFixed(1);
  }
  ...
  getAllStats() {
    ...
    this.players.forEach((stats, name) => {
      result[name] = {
        ...
        totalScope: stats.totalScope,
        averageScope: stats.getAverageScope(),
      };
    });
    return result;
  }
