 ...
 * High-value = face cards, denari, or settebello
 *
...
export function countHighValueCards(capturedCards) {
  ...
  for (const card of capturedCards) {
    ...
    // Any denari card (internal suit key: "denari")
    ...
    if (captures.length > 0) {
      for (const capture of captures) {
        ...
        if (highValueCaptured > 0) {
          ...
          if (isScopa) {
            score += 200; // Bonus for Scopa (but not overwhelming)
          }
    ...
    if (captures.length > 0) {
      for (const capture of captures) {
        ...
        if (tableHasHighValue && captureHasHighValue) {
          ...
          if (isScopa) {
            score += 500; // High bonus for Scopa in endgame
          }
