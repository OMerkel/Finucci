 ...
 * Extends position evaluation with strategic knowledge about Scopa
 *
 * Implements:
 ...
 *
 * Requirements: FR-15.1, FR-15.2, FR-12.4 (Enhanced strategy evaluation)
...
export function scoreSequentialCaptures(move, hand, tableCards) {
  ...
  if (remainingTable.length === 0) {
    // I just cleared the table (Scopa) - massive bonus for control
 ...
 * In endgame, priorities shift: Scope matter more, final card is crucial
 *
...
export function evaluateEndgamePosition(position, move, gameState) {
  ...
  // 1. Scope (clearing table = opponent gets nothing)
  ...
  if (move?.isCapture && move.capturedCards) {
    // Scopa bonus: if we clear table, significant advantage
    if (move.capturedCards.length > 2) {
      endgameBonus += 1.5;
    }

    // Bonus for capturing high-value cards in endgame (settebello, face cards)
    const highValueCaptures = move.capturedCards.filter(
      (c) => c.value > 5 || (c.rank === "7" && c.suit === "denari"),
    ).length;
...
export function evaluatePositionStrategic(
  ...
  if (move) {
    ...
    // NEW: Card value bonus (settebello, denari, high-value cards)
 ...
 * Settebello (7 of denari) is particularly important - worth 1 point by itself
 *
 * @param {Object} move - Move being evaluated
...
export function scoreCardValue(move) {
  ...
  // High bonus for settebello (single point in scoring)
  const hasSettebello = move.capturedCards.some(
    (c) => c.rank === "7" && c.suit === "denari",
  );
  if (hasSettebello) {
    score += 25.0; // Scaled up from 5.0 to ensure it's captured
  }

  // Bonus for capturing denari majority potential (worth 1 point)
  const denariCount = move.capturedCards.filter(
    (c) => c.suit === "denari",
  ).length;
  if (denariCount > 0) {
    score += denariCount * 1.0; // Scaled up from 0.2
  }
