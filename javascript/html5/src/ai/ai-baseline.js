 ...
 * 1. Scopa (capture all table cards) - ALWAYS best, guarantees next turn
 ...
 * @returns {Object} Selected move {card, capture?, isCapture, isScopa}
 */
export function selectBaselineMove(hand, tableCards, gameState) {
  ...
  for (const card of hand) {
    ...
    if (captures.length > 0) {
      ...
      for (const capture of captures) {
        ...
        if (isScopa) {
          // TIER 1: Scopa is always best (clears table, guarantees next move)
          move.quality = 1000;
          ScopaMoves.push(move);
        } else {
  ...
  if (ScopaMoves.length > 0) {
    // Pick first Scopa (all equally good at 1000)
    return ScopaMoves[0];
  }
