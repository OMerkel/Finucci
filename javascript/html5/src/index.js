/**
 * Main entry point for Scopa game application
 */
...
const logger = new Logger("scopa:index");
...
function buildHouseRulesSection(gameController) {
  ...
  row.htmlFor = "options-final-card-Scopa";
  row.innerHTML = `
    <input
      type="checkbox"
      id="options-final-card-Scopa"
      ${gameController.enableFinalCardScopa ? "checked" : ""}
    />
    <span>Score Scopa on final card of round</span>
  `;
  section.appendChild(row);
  ...
  help.textContent =
    "Default: off. A table-clearing capture with your final card of the round does not score a Scopa.";
  section.appendChild(help);

  const deckRow = document.createElement("label");
  deckRow.className = "house-rule-row";
  deckRow.htmlFor = "options-deck-face";
  deckRow.innerHTML = `
    <span>Card faces</span>
    <select id="options-deck-face">
      <option value="carte_merkel" ${gameController.gameView.deckName === "carte_merkel" ? "selected" : ""}>Finucci Italian (default)</option>
      <option value="mazzo_spagnolo" ${gameController.gameView.deckName === "mazzo_spagnolo" ? "selected" : ""}>Mazzo Spagnolo</option>
    </select>
  `;
  section.appendChild(deckRow);

  container.appendChild(section);
}

function readFinalCardScopaFromForm() {
  return Boolean(document.getElementById("options-final-card-Scopa")?.checked);
}

function readDeckFaceFromForm() {
  return document.getElementById("options-deck-face")?.value || "carte_merkel";
}
...
function initSidePanel(gameController) {
    ...
    if (el) {
      ...
      if (id === "overlay-options") {
        ...
        optionsSnapshot = {
          ...
          enableFinalCardScopa: gameController.enableFinalCardScopa,
          deckName: gameController.gameView.deckName,
        };
      }
    }
  }
  ...
  function closeOverlay(id) {
    ...
    if (id === "overlay-options" && optionsSnapshot) {
      ...
      gameController.enableFinalCardScopa = readFinalCardScopaFromForm();
      gameController.gameView.deckName = readDeckFaceFromForm();
      ...
      const changed =
        ...
        optionsSnapshot.enableFinalCardScopa !==
          gameController.enableFinalCardScopa ||
        optionsSnapshot.deckName !== gameController.gameView.deckName;
...
function initGame() {
  ...
  logger.info("Initializing Finucci");
