/**
 * Test suite for round summary overlay controller flow.
 * Requirements: FR-7.1, FR-8.6
 */
...
describe("GameController round summary overlay flow", () => {
  ...
  it("waits for overlay OK before starting next round", async () => {
    // Given: preconditions for "waits for overlay OK before starting next round" are established
  ...
  it("waits for overlay OK before ending game", async () => {
    // Given: preconditions for "waits for overlay OK before ending game" are established
    // When: executing the behavior under test
    // Then: assertions should confirm the expected result
    const { controller, gameView } = makeControllerWithMocks();
    const gate = deferred();
