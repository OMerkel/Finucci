...
export async function playMatch(strategy1, strategy2, config = {}) {
  ...
  const engine = new GameEngine({
    players: ["AI1", "AI2"],
    targetScore: config.targetScore || 11, // Default rules (11 points + 2-point lead)
    ...config,
  });
  ...
  return {
    ...
    player1: {
      ...
      Scope: info1.ScopeWon,
    },
    player2: {
      strategy: strategy2,
      score: info2.score,
      cards: info2.cardsWon,
      Scope: info2.ScopeWon,
    },
    moves: moveCount,
    duration,
...
export async function runTournament(
  ...
  for (let i = 0; i < numMatches; i++) {
    ...
    try {
      ...
      p1Stats.totalScope += result.player1.Scope;
      p2Stats.totalScope += result.player2.Scope;
...
export function printResults(tournamentResults) {
  ...
  console.log("\nTotal Scope:");
  console.log(`  ${strategy1}: ${s1.totalScope} Scope`);
  console.log(`  ${strategy2}: ${s2.totalScope} Scope`);
