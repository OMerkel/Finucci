...
describe("UI Regression Tests - Enhanced Architecture", () => {
  ...
  describe("GameView - Menu Rendering", () => {
    it("should render menu view initially", () => {
      ...
      expect(title.textContent).toContain("Finucci");

      // Should show instructions
      const instructions = container.querySelector(".menu-instructions");
      expect(instructions).not.toBeNull();
      expect(instructions.textContent).toContain("target score");
      expect(instructions.textContent).toContain("Scopa");
    });
  ...
  describe("GameView - Game Board Rendering (REGRESSION: Empty Screen Bug)", () => {
    it("should properly render game board when transitioning from menu", () => {
      ...
      const gameState = {
        currentPlayerIndex: 0,
        players: [
          { score: 0, hand: [new Card("denari", "5", 5)], pile: [] },
          { score: 0, hand: [new Card("coppe", "3", 3)], pile: [] },
        ],
        tableCards: [new Card("spade", "7", 7), new Card("bastoni", "3", 3)],
      };
    ...
    it("should show proper turn indicator (player or AI)", () => {
      ...
      const gameState = {
        currentPlayerIndex: 0,
        players: [
          { score: 5, hand: [new Card("denari", "5", 5)], pile: [] },
          { score: 3, hand: [], pile: [] },
        ],
        tableCards: [],
      };
    ...
    it("should display correct scores for both players", () => {
      ...
      const badgeScores = container.querySelectorAll(
        ".pa-dealer, .pa-captured, .pa-scope",
      );
  ...
  describe("GameBoard - Table and Hand Rendering", () => {
    ...
    it("should render player hand with clickable cards", () => {
      ...
      const gameBoard = new GameBoard(gameState, "mazzo_spagnolo", [
        "human",
        "ai",
      ]);
      const boardElement = gameBoard.createElement();
  ...
  describe("GameBoard - Card Selection", () => {
    it("should track card selection state", () => {
      ...
      const gameState = {
        ...
        players: [
          {
            score: 0,
            hand: [new Card("denari", "5", 5), new Card("coppe", "3", 3)],
            pile: [],
          },
          { score: 0, hand: [], pile: [] },
        ],
    ...
    it("should call onMoveSelected callback when available", () => {
      ...
      // When: assigning the callback on the board instance
  ...
  describe("GameBoard - Hand Visibility Rules (FR-UI-2)", () => {
    it("should keep AI hand face-down in mixed mode while active human is face-up", () => {
      // Given: mixed player types with human as active player
      const gameState = {
        currentPlayerIndex: 0,
        players: [
          {
          ...
          {
            score: 0,
            hand: [new Card("spade", "6", 6), new Card("bastoni", "4", 4)],
            pile: [],
          },
        ],
        tableCards: [],
  ...
  // When: rendering the board for mixed mode
      const board = new GameBoard(gameState, "mazzo_spagnolo", ["human", "ai"]);
      const el = board.createElement();

      const southFaceUp = el.querySelectorAll(".player-area-south .card-element:not(.card-back)");
      const northBacks = el.querySelectorAll(".player-area-north .card-back");

      // Then: active human hand is face-up and AI hand is face-down
      expect(southFaceUp.length).toBe(2);
      expect(northBacks.length).toBe(2);
    });

    it("should hide human hand when not active in mixed mode", () => {
      // Given: mixed player types with AI as active player
      const gameState = {
  ...
  // When: rendering the board while human is inactive
      const board = new GameBoard(gameState, "mazzo_spagnolo", ["human", "ai"]);
      const el = board.createElement();

      const southBacks = el.querySelectorAll(".player-area-south .card-back");
      const southFaceUp = el.querySelectorAll(".player-area-south .card-element:not(.card-back)");

      // Then: human hand is hidden (face-down)
      expect(southBacks.length).toBe(2);
      expect(southFaceUp.length).toBe(0);
    });

    it("should show only active player face-up in same-type mode (AI vs AI)", () => {
      // Given: same-type mode where only current player should be visible
      const gameState = {
        currentPlayerIndex: 1,
        players: [
          {
  ...
  // When: rendering the board for AI vs AI mode
      const board = new GameBoard(gameState, "mazzo_spagnolo", ["ai", "ai"]);
      const el = board.createElement();

      const northFaceUp = el.querySelectorAll(".player-area-north .card-element:not(.card-back)");
      const southBacks = el.querySelectorAll(".player-area-south .card-back");

      // Then: only the active player's cards are face-up
      expect(northFaceUp.length).toBe(2);
      expect(southBacks.length).toBe(2);
    });
  });
  ...
  describe("GameBoard - Table Card Selection (Capture Feature)", () => {
    it("should render table cards as clickable", () => {
      ...
      const gameState = {
        ...
        tableCards: [new Card("coppe", "10", 10), new Card("spade", "2", 2)],
      };
    ...
    it("should display instruction to click table cards", () => {
      ...
      const gameState = {
        ...
        tableCards: [new Card("coppe", "10", 10)],
      };
    ...
    it("should track table card selection state", () => {
      ...
      const tableCard = new Card("coppe", "10", 10);
      const handCard = new Card("denari", "5", 5);
      const gameState = {
        currentPlayerIndex: 0,
        players: [
    ...
    it("should allow multiple table card selection for capture combinations", () => {
      ...
      const handCard = new Card("denari", "5", 5);
      const tableCard1 = new Card("coppe", "3", 3);
      const tableCard2 = new Card("spade", "7", 7);
      const gameState = {
        currentPlayerIndex: 0,
        players: [
          { score: 0, hand: [handCard], pile: [] },
      ...
      // Then: all three cards should be selected for downstream move validation
    ...
    it("should keep only one selected hand card at a time", () => {
      ...
      const handCard1 = new Card("denari", "5", 5);
      const handCard2 = new Card("coppe", "3", 3);
      const gameState = {
        currentPlayerIndex: 0,
        players: [
  ...
  describe("GameController - Capture Move Handling (BUG FIX: Multi-card table selection)", () => {
    it("should construct proper move object for capture with multiple table cards", () => {
      // Given: user selects hand card (8) and two table cards (5, 3)
      const handCard = new Card("denari", "fante", 8);
      const tableCard1 = new Card("coppe", "5", 5);
      const tableCard2 = new Card("spade", "3", 3);

      const selectedCards = [
        { suit: "denari", rank: "fante" }, // Hand card
        { suit: "coppe", rank: "5" }, // Table card 1
        { suit: "spade", rank: "3" }, // Table card 2
      ];
      ...
      expect(handCardFromHand.value).toBe(8);
      ...
      expect(captureSum).toBe(handCardFromHand.value);
      ...
      expect(moveObject.card.value).toBe(8);
      expect(moveObject.capture.length).toBe(2);
      expect(moveObject.isCapture).toBe(true);
    });

    it("should reject capture if table sum does not equal played value", () => {
      // Given: user selects hand card (10) and table cards (5, 3)
      // Capture rule: selected table cards must sum to the played card value
      const handCard = new Card("denari", "re", 10);
      const tableCard1 = new Card("coppe", "5", 5);
      const tableCard2 = new Card("spade", "3", 3);
      ...
      // When: validating the selected capture sum against played card value
      // Then: validation should fail because table sum 8 does not match played value 10
      expect(tableSum).toBe(8);
      expect(tableSum).not.toBe(handCard.value);
    });

    it("should accept valid capture where table sum equals played value", () => {
      // Given: hand card (7) and table cards (5, 2) sum to played card value
      const handCard = new Card("denari", "7", 7);
      const tableCard1 = new Card("spade", "5", 5);
      const tableCard2 = new Card("coppe", "2", 2);

      const captureCards = [tableCard1, tableCard2];
      const tableSum = captureCards.reduce((sum, c) => sum + c.value, 0);

      // When: validating a capture where table sum matches played card value
      // Then: validation should succeed because 5 + 2 = 7
      expect(tableSum).toBe(7);
      expect(tableSum).toBe(handCard.value);
    });
    ...
    it("should handle discard move (no table cards selected)", () => {
      ...
      const selectedCards = [
        { suit: "denari", rank: "5" }, // Only hand card
      ];
