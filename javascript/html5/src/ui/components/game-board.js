...
export class GameBoard {
  ...
  /**
   * Resolve backface SVG path for the currently selected deck.
   */
  getDeckBackImagePath() {
    const backfaceByDeck = {
      carte_merkel: "carte_da_gioco_italiane_back.svg",
      mazzo_spagnolo: "baraja_espanola_back.svg",
      baraja_espanola: "baraja_espanola_back.svg",
    };

    const backfaceFile =
      backfaceByDeck[this.deckName] || `${this.deckName}_back.svg`;
    return `img/deck/${this.deckName}/${backfaceFile}`;
  }
  ...
  createHandArea(playerIdx, position) {
    ...
    const scope =
      this.gameState.stats?.scope?.[playerIdx] ??
      this.gameState.stats?.Scope?.[playerIdx] ??
      0;
    ...
    heading.innerHTML = `
      ...
      <span class="pa-badge pa-scope" title="Scope (sweeps)">🧹 ${scope}</span>
    `;
    ...
    if (hand.length === 0) {
      ...
      for (const card of hand) {
        ...
        img.src = this.getDeckBackImagePath();
        img.alt = "Card back";
  ...
  createPlayArea() {
    ...
    if (remainingCount > 0) {
      ...
      cardBack.src = this.getDeckBackImagePath();
