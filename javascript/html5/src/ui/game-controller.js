...
const logger = new Logger("scopa:game-controller");
...
export class GameController {
  constructor(gameView, eventBus) {
    ...
    this.enableFinalCardScopa = DEFAULT_CONFIG.enableFinalCardScopa;
  ...
  /**
   * Keep Scopa celebration visible before follow-up messages replace it.
   */
  async pauseForScopaToast() {
  ...
  async startNewGame(difficulty) {
    ...
    const gameConfig = createConfiguration({
      enableFinalCardScopa: this.enableFinalCardScopa,
    });
  ...
  async handlePlayerMove(selectedCards) {
    ...
    try {
      ...
      this.gameState = this.gameEngine.gameState;
      this.gameView.setBoardStatus(
        this.buildResolutionStatus(handCard, captureCards, moveResult.isScopa),
        moveResult.moveType === "capture" ? "success" : "info",
      );

      if (moveResult.isScopa) {
        this.gameView.showMessage(GAME_MESSAGES.SCOPA_TOAST, "success");
        await this.pauseForScopaToast();
      }
  ...
  buildPreviewStatus(handCard, captureCards) {
    ...
    const total = captureCards.reduce((sum, c) => sum + c.value, 0);
    const terms = captureCards.map((c) => c.value).join(" + ");
    const handCards =
    ...
    return isScopa
      ? GAME_MESSAGES.PREVIEW_SCOPA(terms, total)
   ...
   */
  buildResolutionStatus(handCard, captureCards, isScopa) {
    if (!captureCards || captureCards.length === 0) {
    ...
    return isScopa ? `${base}${GAME_MESSAGES.RESOLUTION_SCOPA_SUFFIX}` : base;
  }
}
